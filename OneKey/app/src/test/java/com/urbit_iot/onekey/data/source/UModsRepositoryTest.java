...
public class UModsRepositoryTest {
    ...
    private static List<UMod> TASKS = Lists.newArrayList(new UMod("Title1", "Description1"),
            new UMod("Title2", "Description2"));

    private UModsRepository mTasksRepository;

    private TestSubscriber<List<UMod>> mTasksTestSubscriber;

    @Mock
    private UModsDataSource mTasksRemoteDataSource;

    @Mock
    private UModsDataSource mTasksLocalDataSource;
    ...
    public void setupTasksRepository() {
        ...
        mTasksRepository = new UModsRepository(mTasksRemoteDataSource, mTasksLocalDataSource);
    ...
    public void getTasks_repositoryCachesAfterFirstSubscription_whenTasksAvailableInLocalStorage() {
        ...
        TestSubscriber<List<UMod>> testSubscriber1 = new TestSubscriber<>();
        mTasksRepository.getTasks().subscribe(testSubscriber1);

        TestSubscriber<List<UMod>> testSubscriber2 = new TestSubscriber<>();
    ...
    public void saveTask_savesTaskToServiceAPI() {
        ...
        UMod newTask = new UMod(TASK_TITLE, "Some Task Description");
        ...
        assertThat(mTasksRepository.mCachedUMods.size(), is(1));
    }
    ...
    public void completeTask_completesTaskToServiceAPIUpdatesCache() {
        ...
        assertThat(mTasksRepository.mCachedUMods.size(), is(1));
        assertThat(mTasksRepository.mCachedUMods.get(newTask.getId()).isActive(), is(false));
    }
    ...
    public void activateTask_activatesTaskToServiceAPIUpdatesCache() {
        ...
        UMod newTask = new UMod(TASK_TITLE, "Some Task Description", true);
        mTasksRepository.saveTask(newTask);
        ...
        assertThat(mTasksRepository.mCachedUMods.size(), is(1));
        assertThat(mTasksRepository.mCachedUMods.get(newTask.getId()).isActive(), is(true));
    }
    ...
    public void getTask_requestsSingleTaskFromLocalDataSource() {
        ...
        UMod task = new UMod(TASK_TITLE, "Some Task Description", true);
    ...
    public void deleteCompletedTasks_deleteCompletedTasksToServiceAPIUpdatesCache() {
        ...
        UMod newTask = new UMod(TASK_TITLE, "Some Task Description", true);
        mTasksRepository.saveTask(newTask);
        UMod newTask2 = new UMod(TASK_TITLE2, "Some Task Description");
        mTasksRepository.saveTask(newTask2);
        UMod newTask3 = new UMod(TASK_TITLE3, "Some Task Description", true);
        ...
        assertThat(mTasksRepository.mCachedUMods.size(), is(1));
        assertTrue(mTasksRepository.mCachedUMods.get(newTask2.getId()).isActive());
        assertThat(mTasksRepository.mCachedUMods.get(newTask2.getId()).getTitle(), is(TASK_TITLE2));
    }
    ...
    public void deleteAllTasks_deleteTasksToServiceAPIUpdatesCache() {
        ...
        assertThat(mTasksRepository.mCachedUMods.size(), is(0));
    }
    ...
    public void deleteTask_deleteTaskToServiceAPIRemovedFromCache() {
        ...
        UMod newTask = new UMod(TASK_TITLE, "Some Task Description", true);
        mTasksRepository.saveTask(newTask);
        assertThat(mTasksRepository.mCachedUMods.containsKey(newTask.getId()), is(true));
        ...
        assertThat(mTasksRepository.mCachedUMods.containsKey(newTask.getId()), is(false));
    }
    ...
    public void getTasks_refreshesLocalDataSource() {
        ...
        verify(mTasksLocalDataSource, times(TASKS.size())).saveTask(any(UMod.class));
        mTasksTestSubscriber.assertValue(TASKS);
    }

    private void setTasksNotAvailable(UModsDataSource dataSource) {
        when(dataSource.getTasks()).thenReturn(Observable.just(Collections.<UMod>emptyList()));
    }

    private void setTasksAvailable(UModsDataSource dataSource, List<UMod> tasks) {
        // don't allow the data sources to complete.
        when(dataSource.getTasks()).thenReturn(Observable.just(tasks).concatWith(Observable.<List<UMod>>never()));
    }

    private void setTaskNotAvailable(UModsDataSource dataSource, String taskId) {
        when(dataSource.getTask(eq(taskId))).thenReturn(Observable.<UMod>just(null).concatWith(Observable.<UMod>never()));
    }

    private void setTaskAvailable(UModsDataSource dataSource, UMod task) {
        when(dataSource.getTask(eq(task.getId()))).thenReturn(Observable.just(task).concatWith(Observable.<UMod>never()));
    }
}
